namespace DeepSleep.OpenApi
{
    ...
    public class OpenApiYamlFormatter : IFormatStreamReaderWriter
    {
        private readonly IApiRequestContextResolver contextResolver;

        /// <summary>Initializes a new instance of the <see cref="OpenApiYamlFormatter"/> class.</summary>
        /// <param name="contextResolver">The context resolver.</param>
        public OpenApiYamlFormatter(IApiRequestContextResolver contextResolver)
        {
        ...
        public virtual IList<string> WriteableMediaTypes => new[] 
        { 
            "text/x-yaml", 
            "text/yaml", 
            "text/yml", 
        ...
        public async Task<long> WriteType(Stream stream, object obj, IFormatStreamOptions options, Action<long> preWriteCallback = null)
        {
            ...
            if (obj is OpenApiDocument document && document != null)
            {
                using (var ms = new MemoryStream())
                {
                    if (version == "2")
                    {
                        document.Serialize(ms, OpenApiSpecVersion.OpenApi2_0, OpenApiFormat.Yaml);
                    }
                    else
                    {
                        document.Serialize(ms, OpenApiSpecVersion.OpenApi3_0, OpenApiFormat.Yaml);
                    }
