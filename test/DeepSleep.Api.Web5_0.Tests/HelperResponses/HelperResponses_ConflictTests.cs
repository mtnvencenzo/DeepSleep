namespace DeepSleep.Api.Web.Tests.HelperResponses
{
    ...
    public class HelperResponses_ConflictTests : PipelineTestBase
    {
        [Fact]
        public async Task conflict___has_response()
        {
            base.SetupEnvironment();


            var request = @$"
GET https://{host}/helper/responses/conflict HTTP/1.1
Host: {host}
...
";
        ...
        [Fact]
        public async Task conflict___null_response()
        {
            base.SetupEnvironment();


            var request = @$"
GET https://{host}/helper/responses/conflict/null HTTP/1.1
Host: {host}
Connection: keep-alive
User-Agent: UnitTest/1.0 DEV
Accept: {applicationJson}
";
        ...
        [Fact]
        public async Task conflict___null_response_with_errors()
        {
            base.SetupEnvironment();


            var request = @$"
GET https://{host}/helper/responses/conflict/null/with/errors HTTP/1.1
Host: {host}
Connection: keep-alive
User-Agent: UnitTest/1.0 DEV
Accept: {applicationJson}
";
        ...
        [Fact]
        public async Task conflict___has_response_with_headers()
        {
            base.SetupEnvironment();


            var request = @$"
GET https://{host}/helper/responses/conflict/headers HTTP/1.1
Host: {host}
Connection: keep-alive
User-Agent: UnitTest/1.0 DEV
Accept: {applicationJson}
