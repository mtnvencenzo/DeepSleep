namespace DeepSleep.Api.Web.Tests.HelperResponses
{
    ...
    public class HelperResponses_FoundTests : PipelineTestBase
    {
        [Fact]
        public async Task found___has_response()
        {
            base.SetupEnvironment();

            var correlationId = Guid.NewGuid();
            var request = @$"
GET https://{host}/helper/responses/found HTTP/1.1
Host: {host}
...
X-CorrelationId: {correlationId}";
        ...
        [Fact]
        public async Task found___has_response_with_headers()
        {
            base.SetupEnvironment();

            var correlationId = Guid.NewGuid();
            var request = @$"
GET https://{host}/helper/responses/found/headers HTTP/1.1
Host: {host}
Connection: keep-alive
User-Agent: UnitTest/1.0 DEV
Accept: {applicationJson}
