namespace DeepSleep.Api.OpenApiCheckTests.Mocks
{
    ...
    public class MockHttpRequest : HttpRequest, IDisposable
    {
        ...
        public static MockHttpRequest FromHttpRequestMessage(HttpContext context, HttpRequestMessage request, bool includeContentLength = true)
        {
            var method = request.Method.ToString().ToUpper();
            var protocol = request.Version.ToString();
            var rawuri = request.RequestUri.OriginalString;
            ...
            var httpRequest = new MockHttpRequest(context)
            {
            ...
            if (request.Content != null)
            {
                ...
                var writer = new StreamWriter(httpRequest.Body);
                ...
                if (!httpRequest.Headers.ContainsKey("Content-Length"))
                {
                    ...
                    {
                        httpRequest.Headers.Add("Content-Length", $"{httpRequest.Body.Length}");
                    }
                }
            }
            else if(method.ToLower() == "post" || method.ToLower() == "put" || method.ToLower() == "patch")
            {
                if (!httpRequest.Headers.ContainsKey("Content-Length"))
                {
                    if (includeContentLength)
                    {
                        httpRequest.Headers.Add("Content-Length", "0");
                    }
                }
            }
            ...
            if (request.Headers.Count() > 0)
            {
                ...
                {
                    httpRequest.headers.Add(new KeyValuePair<string, StringValues>(header.Key, new StringValues(header.Value.ToArray())));
                }
            }
            ...
            if (httpRequest.headers.ContainsKey("Content-Length"))
            {
                httpRequest.ContentLength = long.Parse(httpRequest.headers["Content-Length"]);
            }
