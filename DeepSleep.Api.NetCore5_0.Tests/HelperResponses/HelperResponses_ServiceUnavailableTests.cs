namespace DeepSleep.Api.NetCore.Tests.HelperResponses
{
    ...
    public class HelperResponses_ServiceUnavailableTests : PipelineTestBase
    {
        [Fact]
        public async Task serviceunavailable___has_response()
        {
            base.SetupEnvironment();

            var correlationId = Guid.NewGuid();
            var request = @$"
GET https://{host}/helper/responses/serviceunavailable HTTP/1.1
Host: {host}
...
X-CorrelationId: {correlationId}";
        ...
        [Fact]
        public async Task serviceunavailable___null_response()
        {
            base.SetupEnvironment();

            var correlationId = Guid.NewGuid();
            var request = @$"
GET https://{host}/helper/responses/serviceunavailable/null HTTP/1.1
Host: {host}
Connection: keep-alive
User-Agent: UnitTest/1.0 DEV
Accept: {applicationJson}
...
X-CorrelationId: {correlationId}";
        ...
        [Fact]
        public async Task serviceunavailable___has_response_with_headers()
        {
            base.SetupEnvironment();

            var correlationId = Guid.NewGuid();
            var request = @$"
GET https://{host}/helper/responses/serviceunavailable/headers HTTP/1.1
Host: {host}
Connection: keep-alive
User-Agent: UnitTest/1.0 DEV
Accept: {applicationJson}
