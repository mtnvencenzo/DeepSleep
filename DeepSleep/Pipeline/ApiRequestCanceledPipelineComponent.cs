...
namespace DeepSleep.Pipeline
{
    ...
    /// 
    /// </summary>
    public class ApiRequestCanceledPipelineComponent
    {
        ...
        public async Task Invoke(IApiRequestContextResolver contextResolver, IApiServiceConfiguration config)
        {
            ...
            if (beforeHook != null)
            {
                var result = await beforeHook.Hook(context, ApiRequestPipelineComponentTypes.RequestCanceledPipelineComponent, ApiRequestPipelineHookPlacements.Before);
            ...
            {
                if (!await context.ProcessHttpRequestCanceled())
                    canContinuePipeline = false;
            }

            if (afterHook != null)
            {
                var result = await afterHook.Hook(context, ApiRequestPipelineComponentTypes.RequestCanceledPipelineComponent, ApiRequestPipelineHookPlacements.After);
                if (result.Continuation == ApiRequestPipelineHookContinuation.ByPassComponentAndCancel || result.Continuation == ApiRequestPipelineHookContinuation.InvokeComponentAndCancel)
                    canContinuePipeline = false;
            }
