...
namespace DeepSleep.Pipeline
{
    ...
    /// 
    /// </summary>
    public class ApiRequestEndpointInvocationPipelineComponent
    {
        ...
        public async Task Invoke(IApiRequestContextResolver contextResolver, IApiServiceConfiguration config)
        {
            ...
            if (beforeHook != null)
            {
                var result = await beforeHook.Hook(context, ApiRequestPipelineComponentTypes.RequestEndpointInvocationPipeline, ApiRequestPipelineHookPlacements.Before);
            ...
            {
                if (!await context.ProcessHttpEndpointInvocation())
                    canContinuePipeline = false;
            }

            if (afterHook != null)
            {
                var result = await afterHook.Hook(context, ApiRequestPipelineComponentTypes.RequestEndpointInvocationPipeline, ApiRequestPipelineHookPlacements.After);
                if (result.Continuation == ApiRequestPipelineHookContinuation.ByPassComponentAndCancel || result.Continuation == ApiRequestPipelineHookContinuation.InvokeComponentAndCancel)
                    canContinuePipeline = false;
            }
